...
class UserFrom extends React.Component {
  ...
  constructor(props){
    ...
    this.AddJuese = this.AddJuese.bind(this);
  ...
  componentDidMount() {
    ...
    if(that.state.data == 5 || that.state.data == 0) {
      ...
      axios.post(url +'/config/community/unused',data)
      .then(function(res){
          if(res.data.resultCode === 1000) {
            console.log(res.data)
            that.setState({
  ...
  AddJuese = () => {
    ...
    axios.post(url +'/config/actor/list',data)
      .then(function(res){
        console.log(res.data)
          if(res.data.resultCode === 1000) {
            that.setState({
                juese: res.data.resultData,
              })
          } else {
              message.error('失败!');
          }
  ...
  AddSite = (Id) => {
    const that = this;
    axios.post(url +'/config/department/list',{
      userId: Id
    })
      .then(function(res){
        console.log(res.data)
  ...
  render() {
        ...
        <Form className="From" onSubmit={this.handleSubmit}>
          ...
          >
            {getFieldDecorator('loginName', {
              rules: [{
                pattern: /^[0-9a-zA-Z]*$/g, message: '请输入数字或字母的类型',
            },{
              max: 12, message: '用户名不超过12位',
            },{
              min: 4, message: '用户名不低于4位',
            },{ required: true, message: '请填写你的用户名!', whitespace: true }],
            })(
              <Input />
            )}
          </FormItem>
    ...
    } else if(this.state.data == 3) {//单位新增
        ...
        <Form className="From" onSubmit={this.handleSubmit}>
          ...
          >
            {getFieldDecorator('password', {
              ...
              min: 6, message: '用户名不低于6位',
            },{ required: true, message: '请填写你的密码!', whitespace: true }],
            })(
              <Input />
            )}
          </FormItem>
...
const Mfrom = Form.create()(UserFrom);
